...
using eRestraunt.Entities.DTOs;
using System;
    ...
    public class SeatingController
    {
        ...
        public List<SeatingSummary> SeatingByDateTime(DateTime date, TimeSpan time)
        {
            using (var context = new RestrauntContext())
            {
                            ...
                            select new
                            {
                                ...
                                Bills = from billing in data.Bills
                                        where
                                            ...
                                            // "The specified type member 'TomeOfDay' is not supported..."
                                            //&& billing.BillDate.TimeOfDay <= time
                                                    && DbFunctions.CreateTime(billing.BillDate.Hour,
                                                    billing.BillDate.Minute, billing.BillDate.Second) <= time
                ...
                // Setp 2 - Union the walk-in bills and the reservation bills while extracting the relevant bill info
